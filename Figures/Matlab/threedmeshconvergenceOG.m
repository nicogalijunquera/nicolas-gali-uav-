%% 3D Mesh Independence Study – Enhanced Plot (Updated Data)
...
elements = [1.8968e6, 4.9017e6, 7.3286e6, 1.0366e7];
drag     = [7.0669, 6.9469, 6.8967, 6.8479];
lift     = [122.43, 119.86, 116.65, 116.35];
x        = 1:numel(meshID);
...
% Clean transition arrays (indices 2,3,4 → positions 1,2,3)
ld = lift_diff(2:end);
dd = drag_diff(2:end);
...
subtitle(tl,'Baseline BrUAS 2022  |  RANS CFD  |  Successive mesh refinement', ...
...
fill(ax2, [x fliplr(x)], ...
    [drag ones(1,numel(x))*(min(drag)-0.02)], ...
...
ax2.YLim = [min(drag)-0.06  max(drag)+0.08];
...
% Value labels – computed directly from ld/dd, offset scales with data
...
leg.Title.FontSize   = 13;
leg.Title.FontWeight = 'bold';

%% ── Export ───────────────────────────────────────────────────────────────
exportgraphics(fig, 'mesh_independence_3D_v2.png', 'Resolution',300);
exportgraphics(fig, 'mesh_independence_3D_v2.pdf', 'ContentType','vector');
fprintf('Saved mesh_independence_3D_v2.png and .pdf\n');